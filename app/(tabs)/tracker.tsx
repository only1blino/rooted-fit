...
export default function TrackerScreen() {
  ...
  useEffect(() => {
    Promise.all([loadProfile(), loadCheckIns(), loadMeasurements(), loadProgressPhotos()]).then(([savedProfile, savedCheckIns, savedMeasurements, savedPhotos]) => {
      ...
      if (latest) {
        ...
        setUpperArmText(latest.upperArmCm ? String(latest.upperArmCm) : "");
        setThighText(latest.thighCm ? String(latest.thighCm) : "");
      }
  ...
  const saveMeasurement = async () => {
    ...
    const length = (value: number | null) => value === null ? null : isImperial ? value * 2.54 : value;
    const weightKg = rawWeight === null ? null : isImperial ? rawWeight / 2.20462 : rawWeight;
    const entry: BodyMeasurement = { id: `measurement-${Date.now()}`, date: formatToday(), weightKg, waistCm: length(rawWaist), hipCm: length(rawHip), chestCm: length(rawChest), upperArmCm: length(rawUpperArm), thighCm: length(rawThigh), unit: profile?.measurementUnit ?? "metric", note: measurementNote.trim() };
  ...
  const lengthUnit = profile?.measurementUnit === "imperial" ? "in" : "cm";
  const weightUnit = profile?.measurementUnit === "imperial" ? "lb" : "kg";
  const displayLength = (value: number | null) => value === null ? "Not logged" : profile?.measurementUnit === "imperial" ? (value / 2.54).toFixed(1) : String(value);
  const displayWeight = (value: number | null) => value === null ? "Not logged" : profile?.measurementUnit === "imperial" ? (value * 2.20462).toFixed(1) : String(value);
