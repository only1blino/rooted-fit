...
import { loadProfile, type UserProfile } from "@/lib/rootedfit-profile";
...
export default function WorkoutsScreen() {
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.screen}><ScrollView contentContainerStyle={styles.content}><Text style={styles.eyebrow}>YOUR HOME WORKOUT LIBRARY</Text><Text style={styles.title}>{TITLE[goal]}</Text><Text style={styles.body}>These external follow-along sessions are selected around your primary focus and home-based movement. Choose what feels appropriate for your time, space, and energy today.</Text>{resources.map((item) => <View key={item.title} style={styles.card}><Text style={styles.cardKicker}>{item.format.toUpperCase()} · {item.duration.toUpperCase()}</Text><Text style={styles.cardTitle}>{item.title}</Text><Text style={styles.cardCopy}>{item.why}</Text><Pressable onPress={() => open(item.url)} style={styles.openButton}><Text style={styles.openButtonText}>Open YouTube session →</Text></Pressable></View>)}<View style={styles.note}><Text style={styles.noteText}>These are optional resources, not medical instruction. Pause, modify, or choose a gentler session if anything does not feel right. The Extras tab has the 10-minute timer and desk-stretch tools.</Text></View></ScrollView></ScreenContainer>;
}

const styles = StyleSheet.create({
  screen: { backgroundColor: "#F8F6EF" }, loading: { alignItems: "center", backgroundColor: "#F8F6EF", justifyContent: "center" }, content: { padding: 22 }, eyebrow: { color: "#2D6A4F", fontSize: 11, fontWeight: "800", letterSpacing: 1.2, marginBottom: 10 }, title: { color: "#1F2A25", fontSize: 30, fontWeight: "800", letterSpacing: -0.5, lineHeight: 36 }, body: { color: "#526259", fontSize: 16, lineHeight: 24, marginBottom: 20, marginTop: 10 }, card: { backgroundColor: "#FFFFFF", borderColor: "#DDE5DA", borderRadius: 18, borderWidth: 1, marginBottom: 13, padding: 17 }, cardKicker: { color: "#2D6A4F", fontSize: 10, fontWeight: "800", letterSpacing: 1, marginBottom: 7 }, cardTitle: { color: "#1F2A25", fontSize: 19, fontWeight: "800", lineHeight: 25 }, cardCopy: { color: "#526259", fontSize: 14, lineHeight: 21, marginTop: 7 }, openButton: { alignItems: "center", backgroundColor: "#EAF3EA", borderRadius: 12, marginTop: 15, minHeight: 46, justifyContent: "center", paddingHorizontal: 12 }, openButtonText: { color: "#1D583E", fontSize: 14, fontWeight: "800", textDecorationLine: "underline" }, note: { backgroundColor: "#FCF3E7", borderColor: "#E8D1A4", borderRadius: 16, borderWidth: 1, marginTop: 5, padding: 15 }, noteText: { color: "#6B4A2C", fontSize: 13, lineHeight: 19 },
});
