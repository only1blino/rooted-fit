...
export default function WorkoutsScreen() {
  ...
  const [sessionReminder, setSessionReminder] = useState<PlannedSessionReminder>({ time: null, weekdays: [2, 3, 4, 5, 6], enabled: false, notificationIds: [], updatedAt: "" });
  ...
  const updateSessionReminder = async (time: string | null, weekdays = sessionReminder.weekdays) => {
    if (Platform.OS !== "web" && sessionReminder.notificationIds.length) await Promise.all(sessionReminder.notificationIds.map((id) => Notifications.cancelScheduledNotificationAsync(id).catch(() => undefined)));
    if (!time || !weekdays.length) {
      const next = { time, weekdays, enabled: false, notificationIds: [], updatedAt: new Date().toISOString() };
      setSessionReminder(next);
      await savePlannedSessionReminder(next);
      setReminderStatus(time ? "Choose at least one day to schedule this reminder." : "No session reminder is planned.");
      return;
    }
    if (Platform.OS === "web") {
      const next = { time, weekdays, enabled: true, notificationIds: [], updatedAt: new Date().toISOString() };
      setSessionReminder(next);
      await savePlannedSessionReminder(next);
      setReminderStatus(`Gentle reminder saved for ${reminderDaysLabel(weekdays)} at ${time}. Browser beta keeps this plan on this device; native app reminders can appear outside the app after permission.`);
      return;
    }
    ...
    const notificationIds = await Promise.all(weekdays.map((weekday) => Notifications.scheduleNotificationAsync({ content: { title: "RootedFit movement check-in", body: `Your planned ${workout.durationMinutes}-minute ${workout.category.toLowerCase()} session is ready when you are.`, data: { url: "/workouts" }, sound: false }, trigger: { type: Notifications.SchedulableTriggerInputTypes.WEEKLY, weekday, hour, minute, channelId: Platform.OS === "android" ? "rootedfit-sessions" : undefined } })));
    const next = { time, weekdays, enabled: true, notificationIds, updatedAt: new Date().toISOString() };
    setSessionReminder(next);
    await savePlannedSessionReminder(next);
    setReminderStatus(`A gentle ${reminderDaysLabel(weekdays)} reminder is scheduled for ${time} on this device.`);
  };
  const toggleReminderWeekday = async (weekday: ReminderWeekday) => {
    const weekdays = sessionReminder.weekdays.includes(weekday) ? sessionReminder.weekdays.filter((day) => day !== weekday) : [...sessionReminder.weekdays, weekday].sort((a, b) => a - b) as ReminderWeekday[];
    await updateSessionReminder(sessionReminder.time, weekdays);
  };
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.screen}><ScrollView contentContainerStyle={styles.content}>
    ...
    <View style={styles.reminderCard}><Text style={styles.cardKicker}>GENTLE SESSION REMINDER</Text><Text style={styles.reminderCopy}>Choose the days and time that fit your real week. This is a supportive check-in, not a streak target.</Text><Text style={styles.reminderSectionLabel}>REMINDER DAYS</Text><View style={styles.weekdayOptions}>{WEEKDAY_OPTIONS.map((day) => <Pressable key={day.value} onPress={() => toggleReminderWeekday(day.value)} style={[styles.weekdayOption, sessionReminder.weekdays.includes(day.value) && styles.weekdayOptionActive]}><Text style={[styles.weekdayOptionText, sessionReminder.weekdays.includes(day.value) && styles.weekdayOptionTextActive]}>{day.label}</Text></Pressable>)}</View><Text style={styles.reminderSectionLabel}>REMINDER TIME</Text><View style={styles.reminderOptions}>{REMINDER_OPTIONS.map((option) => <Pressable key={option.label} onPress={() => updateSessionReminder(option.time)} style={[styles.reminderOption, sessionReminder.time === option.time && styles.reminderOptionActive]}><Text style={[styles.reminderOptionText, sessionReminder.time === option.time && styles.reminderOptionTextActive]}>{option.label}</Text></Pressable>)}</View>{reminderStatus ? <Text style={styles.reminderStatus}>{reminderStatus}</Text> : sessionReminder.enabled && sessionReminder.time ? <Text style={styles.reminderStatus}>Planned for {reminderDaysLabel(sessionReminder.weekdays)} at {sessionReminder.time} on this device.</Text> : null}</View>
