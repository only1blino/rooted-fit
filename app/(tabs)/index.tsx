...
import { COUNTRY_OPTIONS, suggestedFoods, suggestedFruits } from "@/lib/food-catalogue";
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    loadProfile().then((saved) => {
      if (saved) {
        ...
        const showLength = (value: number | null) => value === null ? "" : saved.measurementUnit === "imperial" ? (value / 2.54).toFixed(1) : String(value);
        const showWeight = (value: number | null) => value === null ? "" : saved.measurementUnit === "imperial" ? (value * 2.20462).toFixed(1) : String(value);
        setNumbers({ electricity: String(saved.electricityHoursPerDay || ""), market: String(saved.marketMinutesAway || ""), steps: String(saved.dailyStepCount || ""), movement: String(saved.workoutMinutesPerDay || ""), height: showLength(saved.heightCm), weight: showWeight(saved.weightKg), waist: showLength(saved.baselineWaistCm), hip: showLength(saved.baselineHipCm), chest: showLength(saved.baselineChestCm) });
        setStep(6);
      }
  ...
  const displayMeasurement = (value: number | null, kind: "length" | "weight", unit = profile.measurementUnit) => {
    ...
    if (unit === "metric") return String(value);
    return kind === "weight" ? (value * 2.20462).toFixed(1) : (value / 2.54).toFixed(1);
  };
  const updateMeasurementNumber = (key: string, rawValue: string, profileKey: keyof UserProfile, kind: "length" | "weight") => {
    setNumbers((current) => ({ ...current, [key]: rawValue }));
    const parsed = numberOrNull(rawValue);
    const canonical = parsed === null ? null : profile.measurementUnit === "imperial" ? kind === "weight" ? parsed / 2.20462 : parsed * 2.54 : parsed;
  ...
  const lengthUnit = profile.measurementUnit === "metric" ? "cm" : "in";
  const weightUnit = profile.measurementUnit === "metric" ? "kg" : "lb";
