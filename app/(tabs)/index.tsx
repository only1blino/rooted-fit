...
import { COUNTRY_OPTIONS, heritageMealSuggestions, locationSuggestionLabel, resolveFoodLocation, seasonalFoodCues, suggestedFoods, suggestedFruits, suggestedMeals } from "@/lib/food-catalogue";
...
export default function HomeScreen() {
  ...
  const locationLabel = locationSuggestionLabel(profile.country, profile.city, profile.cityCountryMatchChoice);
  const detectedLocation = resolveFoodLocation(profile.country, profile.city);
  const resolvedLocation = resolveFoodLocation(profile.country, profile.city, profile.cityCountryMatchChoice);
  const seasonCue = seasonalFoodCues(profile.country, profile.city, new Date(), profile.cityCountryMatchChoice);
  const foodSuggestions = suggestedFoods(profile.country, profile.city, profile.cityCountryMatchChoice).filter((item) => item.toLowerCase().includes(foodQuery.toLowerCase()));
  const mealSuggestions = suggestedMeals(profile.country, profile.city, profile.cityCountryMatchChoice);
  const heritageMealIdeas = heritageMealSuggestions(profile.foodHeritagePreferences);
  ...
  const toggleFoodHeritage = (country: UserProfile["country"]) => setProfile((current) => ({ ...current, foodHeritagePreferences: current.foodHeritagePreferences.includes(country) ? current.foodHeritagePreferences.filter((item) => item !== country) : [...current.foodHeritagePreferences, country] }));
  const updateCity = (city: string) => setProfile((current) => ({ ...current, city, cityCountryMatchChoice: "auto" }));
  const confirmDetectedCityCountry = () => setProfile((current) => ({ ...current, country: detectedLocation.country, cityCountryMatchChoice: "confirmed", localIngredients: [], favoriteFruits: [] }));
  const chooseManualCityCountry = () => setProfile((current) => ({ ...current, cityCountryMatchChoice: "manual", localIngredients: [], favoriteFruits: [] }));
  const chooseCountry = (country: UserProfile["country"]) => setProfile((current) => ({ ...current, country, cityCountryMatchChoice: detectedLocation.isCityMatch && country === detectedLocation.country ? "confirmed" : "manual", localIngredients: [], favoriteFruits: [] }));
  ...
  const continueOnboarding = () => {
    ...
    if (step === 1 && detectedLocation.isCityMatch && profile.cityCountryMatchChoice === "auto") return Alert.alert("Confirm your city match", `We found ${profile.city.trim()}, ${detectedLocation.country}. Use the confirmation button if this is right, or choose another country if it is not.`);
    ...
    <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.screen}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          {step === 1 ? <View style={styles.content}><Text style={styles.title}>Start with your daily reality.</Text><Text style={styles.body}>Country, electricity, and market access shape what can be prepared, stored, and bought without waste.</Text><TextField label="City or community" value={profile.city} onChangeText={updateCity} placeholder="e.g. Accra" />{detectedLocation.isCityMatch ? <View style={styles.locationCue}><Text style={styles.locationCueKicker}>CHECK YOUR LOCATION MATCH</Text><Text style={styles.locationCueTitle}>We found {profile.city.trim()}, {detectedLocation.country}</Text><Text style={styles.locationCueText}>Confirm this only if it is right. This chooses city-local recipes and ingredients; it never assumes your heritage or favourite cuisine.</Text><View style={styles.choiceStack}><ChoiceButton label={`Use ${detectedLocation.country} for ${profile.city.trim()}`} description={profile.cityCountryMatchChoice === "confirmed" ? "Confirmed for this local recipe pack." : "Use this audited city-country match."} selected={profile.cityCountryMatchChoice === "confirmed" && profile.country === detectedLocation.country} onPress={confirmDetectedCityCountry} /><ChoiceButton label="Choose a different country" description="Keep the city text, but use the country you select below instead." selected={profile.cityCountryMatchChoice === "manual"} onPress={chooseManualCityCountry} /></View></View> : <View style={styles.locationCue}><Text style={styles.locationCueKicker}>CITY NOT YET IN THE VERIFIED MAP</Text><Text style={styles.locationCueTitle}>Choose the country that fits your location</Text><Text style={styles.locationCueText}>We will use your selected country starter pack and the foods you personally confirm. You can correct this any time.</Text></View>}<Text style={styles.fieldLabel}>{profile.cityCountryMatchChoice === "manual" ? "Country you want to use" : "Country food catalogue"}</Text><Text style={styles.helper}>{profile.cityCountryMatchChoice === "manual" ? "Your selection overrides the automatic city-country match for this plan." : "This powers suggested foods and fruits. Every selection stays fully editable."}</Text><View style={styles.choiceStack}>{COUNTRY_OPTIONS.map((country) => <ChoiceButton key={country} label={country} selected={profile.country === country} onPress={() => chooseCountry(country)} />)}</View><NumberField label="Typical electricity access" value={numbers.electricity} onChange={(value) => updateNumber("electricity", value, "electricityHoursPerDay", 24)} suffix="hours each day" helper="You can type a number directly or use the controls." /></View> : null}
...
const styles = StyleSheet.create({
  ...
  choiceStack: { gap: 9, marginBottom: 23 }, choice: { backgroundColor: "#FFFFFF", borderColor: "#DDE5DA", borderRadius: 14, borderWidth: 1, minHeight: 50, paddingHorizontal: 15, paddingVertical: 13 }, choiceSelected: { backgroundColor: "#EAF3EA", borderColor: "#2D6A4F" }, choicePressed: { opacity: 0.74 }, choiceCopy: { flex: 1 }, choiceText: { color: "#385046", fontSize: 15, fontWeight: "600" }, choiceTextSelected: { color: "#1D583E", fontWeight: "800" }, choiceDescription: { color: "#6B7A70", fontSize: 12, lineHeight: 17, marginTop: 4 }, locationCue: { backgroundColor: "#EAF3EA", borderColor: "#B7D7BE", borderRadius: 16, borderWidth: 1, marginBottom: 22, padding: 15 }, locationCueKicker: { color: "#2D6A4F", fontSize: 10, fontWeight: "800", letterSpacing: 1, marginBottom: 5 }, locationCueTitle: { color: "#1D583E", fontSize: 17, fontWeight: "800", lineHeight: 22 }, locationCueText: { color: "#405247", fontSize: 13, lineHeight: 19, marginTop: 5 }, dropdownButton: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#DDE5DA", borderRadius: 14, borderWidth: 1, flexDirection: "row", justifyContent: "space-between", marginBottom: 8, minHeight: 52, paddingHorizontal: 15 }, dropdownText: { color: "#1F2A25", fontSize: 15, fontWeight: "700" }, dropdownArrow: { color: "#2D6A4F", fontSize: 18, fontWeight: "800" }, dropdownMenu: { gap: 8, marginBottom: 18 }, sectionTitle: { color: "#1F2A25", fontSize: 20, fontWeight: "800", lineHeight: 27, marginBottom: 8, marginTop: 6 },
