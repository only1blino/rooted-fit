...
import { ActivityIndicator, Alert, Animated, Image, Modal, Platform, Pressable, ScrollView, Share, StyleSheet, Text, View } from "react-native";
...
import { buildWeeklyPlan, findSimilarRecipe, formatGroceryChecklistPrintHtml, formatGroceryListExport, loadGroceryChecklist, loadMealSwaps, loadProfile, loadRecipePhotoFavorites, saveGroceryChecklist, saveMealSwaps, saveProfile, saveRecipePhotoFavorites, subscribeProfile, upsertCityRecipeRating, type GroceryChecklistItem, type LocalRecipePhotoFavorite, type MealFrequency, type MealSwap, type ServingSize, type UserProfile } from "@/lib/rootedfit-profile";
...
export default function MealsScreen() {
  ...
  const [photoFavorites, setPhotoFavorites] = useState<LocalRecipePhotoFavorite[]>([]);
  const [expandedPhoto, setExpandedPhoto] = useState<{ title: string; city: string; url: string } | null>(null);
  ...
  useEffect(() => {
    ...
    loadRecipePhotoFavorites().then(setPhotoFavorites);
  ...
  const isPhotoFavorite = (recipeTitle: string) => photoFavorites.some((favorite) => favorite.recipeTitle === recipeTitle && favorite.city.toLowerCase() === profile?.city.trim().toLowerCase());
  const toggleRecipePhotoFavorite = async (recipeTitle: string, thumbnailUrl: string) => {
    if (!profile) return;
    const exists = isPhotoFavorite(recipeTitle);
    const next = exists
      ? photoFavorites.filter((favorite) => !(favorite.recipeTitle === recipeTitle && favorite.city.toLowerCase() === profile.city.trim().toLowerCase()))
      : [{ recipeTitle, city: profile.city.trim() || profile.country, thumbnailUrl, savedAt: new Date().toISOString() }, ...photoFavorites];
    setPhotoFavorites(next);
    await saveRecipePhotoFavorites(next);
  };
  ...
  </ScrollView><Modal visible={Boolean(expandedPhoto)} transparent animationType="fade" onRequestClose={() => setExpandedPhoto(null)}><Pressable style={styles.photoOverlay} onPress={() => setExpandedPhoto(null)}><View style={styles.photoModal}><Image source={{ uri: expandedPhoto?.url }} style={styles.expandedPhoto} resizeMode="cover" /><Text style={styles.expandedPhotoCity}>{expandedPhoto?.city}</Text><Text style={styles.expandedPhotoTitle}>{expandedPhoto?.title}</Text><Text style={styles.expandedPhotoHint}>Tap outside to close</Text></View></Pressable></Modal><CompletionRatingPrompt visible={mealRatingVisible} completionKey={`meal:${profile.rotationWeek}:${day.day}`} title="today’s meal plan" onClose={() => setMealRatingVisible(false)} /></ScreenContainer>;
}
...
Object.assign(styles, {
  recipeThumbnailStack: { alignItems: "center", gap: 5 },
  photoSaveButton: { alignItems: "center", backgroundColor: "#F1F7F0", borderColor: "#BCD4BF", borderRadius: 8, borderWidth: 1, paddingHorizontal: 6, paddingVertical: 4 },
  photoSaveButtonActive: { backgroundColor: "#2D6A4F", borderColor: "#2D6A4F" },
  photoSaveText: { color: "#1D583E", fontSize: 10, fontWeight: "800" },
  photoSaveTextActive: { color: "#FFFFFF" },
  photoOverlay: { alignItems: "center", backgroundColor: "rgba(20, 35, 27, 0.76)", flex: 1, justifyContent: "center", padding: 24 },
  photoModal: { backgroundColor: "#FFFFFF", borderRadius: 22, maxWidth: 420, overflow: "hidden", padding: 14, width: "100%" },
  expandedPhoto: { backgroundColor: "#EAF3EA", borderRadius: 14, height: 310, width: "100%" },
  expandedPhotoCity: { color: "#2D6A4F", fontSize: 11, fontWeight: "800", letterSpacing: 0.8, marginTop: 12 },
  expandedPhotoTitle: { color: "#1F2A25", fontSize: 20, fontWeight: "800", lineHeight: 26, marginTop: 4 },
  expandedPhotoHint: { color: "#6B7A70", fontSize: 12, marginTop: 8 },
});
