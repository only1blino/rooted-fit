...
export function TesterTools() {
  ...
  const copyFeedback = async () => {
    const draft = `RootedFit tester feedback\n\n${feedback.trim() || "[Add your feedback here]"}\n\nPage: ${currentWebUrl()}`;
    if (await copyText(draft)) {
      Alert.alert("Feedback copied", "Paste it into the feedback channel your test coordinator shared with you.");
    } else {
      Alert.alert("Feedback draft", draft);
    }
  };
  ...
  return <View pointerEvents="box-none" style={styles.overlay}>
    ...
    <Modal animationType="fade" transparent visible={feedbackVisible} onRequestClose={() => setFeedbackVisible(false)}>
      <View style={styles.backdrop}><View style={styles.modalCard}><Text style={styles.kicker}>TESTER FEEDBACK</Text><Text style={styles.modalTitle}>What should improve?</Text><Text style={styles.modalBody}>Describe a bug, confusing step, missing food option, or idea. Your feedback remains in this draft until you copy it.</Text><TextInput accessibilityLabel="Feedback message" value={feedback} onChangeText={setFeedback} multiline placeholder="For example: I expected the grocery checklist to..." placeholderTextColor="#8B978F" style={styles.feedbackInput} textAlignVertical="top" /><View style={styles.actionRow}><Pressable accessibilityRole="button" onPress={() => setFeedbackVisible(false)} style={styles.secondaryButton}><Text style={styles.secondaryButtonText}>Cancel</Text></Pressable><Pressable accessibilityRole="button" onPress={copyFeedback} style={styles.primaryButton}><Text style={styles.primaryButtonText}>Copy feedback</Text></Pressable></View></View></View>
    </Modal>
  </View>;
}
